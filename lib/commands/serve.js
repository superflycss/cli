...
module.exports = cli => {
  cli
    .command("serve")
    .alias("s")
    .description("Compile, serve, and watch test md, svg, css (Also main css) and test html")
    .action(name => {
...
function runBuild() {
  return new Promise( async (resolve) => {
    runTargetBuild().then(async ()=>{
      ...
      log('info', 'Deleting serve/css/**/*.css');
      del.sync(PLI.SERVE_TEST_CSS);
      await copyTargetTestCSS(PLI.serve.css);
      log('info', 'Deleting serve/**/*.html');
      del.sync(PLI.SERVE_TEST_HTML);
      await copyTargetTestHTML(PLI.SERVE);  
      resolve();
    });
  });
}
